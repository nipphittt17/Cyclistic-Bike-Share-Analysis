...
### Download and merge the datasets from January to June
...
filtered_check <- filter(bike_data, ended_at < started_at)
```


### Exclude entries with ride durations longer than a day or less than a minute.
```{r exclude entries 2}
# ride_length are longer than a day (24 hrs).
bike_data <- bike_data %>% 
  filter(!(difftime(ended_at, started_at) > 24*60*60))
filtered_check2 <- filter(bike_data, difftime(ended_at, started_at) > 24*60*60)

# ride_length are less than a minute.
